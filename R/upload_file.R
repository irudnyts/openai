...
#'   file to be uploaded. If the `purpose` is set to "search" or "answers", each
#'   line is a JSON record with a "text" field and an optional "metadata" field.
#'   Only "text" field will be used for search. Specially, when the `purpose` is
#'   "answers", "\\n" is used as a delimiter to chunk contents in the "text"
#'   field into multiple documents for finer-grained matching. If the `purpose`
#'   is set to "classifications", each line is a JSON record representing a
#'   single training example with "text" and "label" fields along with an
#'   optional "metadata" field. If the `purpose` is set to "fine-tune", each
#'   line is a JSON record with "prompt" and "completion" fields representing
#'   your [training
#'   examples](https://beta.openai.com/docs/guides/fine-tuning/prepare-training-data).
#'
#' @param purpose required; defaults to `"search"`; a length one character
#'   vector, one among `"search"`, `"answers"`, `"classifications"`, and
#'   `"fine-tune"`. The intended purpose of the uploaded documents. This allows
