...
#' [this page](https://beta.openai.com/docs/api-reference/completions/create)
#' for details.
#'
...
#' @param prompt required; defaults to `"<|endoftext|>"`; an arbitrary
...
#' @param suffix optional; defaults to `NULL`; a length one
...
#' prompt plus `max_tokens` cannot exceed the model's context length. Most
#' models have a context length of `2048` tokens (except for the newest
#' models, which support `4096`).
#' @param temperature required; defaults to `1`; a length one numeric
...
#' `0.9` for more creative applications, and `0` (argmax sampling) for
#' ones with a well-defined answer. We generally recommend altering (i.e.,
#' setting the value different from `1`) this or `top_p` but not both.
#' @param top_p required; defaults to `1`; a length one numeric
#' vector with the value between `0` and `1`. An alternative to
...
#' `1`) this or `temperature` but not both.
#' @param n required; defaults to `1`; a length one numeric vector with the
#' integer value greater than `0`. How many completions to generate for
#' each prompt. **Note:** Because this parameter generates many
...
#' `stop`.
#' @param stream required; defaults to `FALSE`; a length one logical
...
#' terminated by a `data: [DONE]` message. **Currently is not
#' implemented.**
...
#' For example, if `logprobs` is `5`, the API will return a list of
#' the 5 most likely tokens. The API will always return the `logprob` of
#' the sampled token, so there may be up to `logprobs+1` elements in the
#' response. The maximum value for logprobs is `5`. If you need more than
#' this, please contact \email{support@openai.com} and describe your use case.
#' @param echo required; defaults to `FALSE`; a length one logical
...
#' vector with a value between `-2` and `2`. Positive values penalize
...
#' @param best_of required; defaults to `1`; a length one numeric vector
#' with the integer value greater than `0`. Generates `best_of`
#' completions server-side and returns the "best" (the one with the lowest log
#' probability per token). Results cannot be streamed. When used with `n`,
